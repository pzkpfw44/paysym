...
function exportToPdf() {
    ...
    const results = calculateTotalPayout(quarterlyAchievements, monthlySales, fte);
    
    // Add results data
    doc.setFontSize(10);
    doc.text(`Average Achievement: ${formatPercent(results.avgAchievement)}`, 20, y);
    y += 7;
    doc.text(`Total Payout: ${formatCurrency(results.totalPayout)}`, 20, y);
    y += 7;
    doc.text(`Commission: ${formatCurrency(results.totalCommission)}`, 20, y);
    y += 7;
    doc.text(`Quarterly Bonus: ${formatCurrency(results.totalQuarterlyBonus)}`, 20, y);
    y += 7;
    doc.text(`Continuity Bonus: ${formatCurrency(results.totalContinuityBonus)}`, 20, y);
    y += 15;
    ...
    for (let i = 0; i < 4; i++) {
        ...
        tableRows.push([
            `Q${i+1}`,
            formatPercent(quarterlyAchievements[i]),
            formatCurrency(results.quarterlyBonuses[i]),
            formatCurrency(results.continuityBonuses[i]),
    ...
    // Get elasticity data
    const elasticityData = calculateElasticityPerRange();
    const rangeNames = [
        '0% - 40%',
        '41% - 70%',
        '71% - 89%',
    ...
    for (let i = 0; i < rangeNames.length; i++) {
        ...
        elasticityRows.push([
            rangeNames[i],
            formatCurrency(elasticityData[rangeKey].elasticity),
            formatCurrency(elasticityData[rangeKey].revenuePerPoint),
            formatRatio(elasticityData[rangeKey].roi)
        ]);
    }
